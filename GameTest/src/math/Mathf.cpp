...
float Mathf::ClampFloat(const float value)
{
...
{
	return a + (b - a) * Mathf::ClampFloat(t);
}
...
float Mathf::LerpAngle(const float a, const float b, const float t)
{
	...
	return a + num * Mathf::ClampFloat(t);
}
...
float Mathf::InverseLerp(const float a, const float b, const float value)
{
	if (static_cast<double>(a) != static_cast<double>(b))
	{
		return Mathf::ClampFloat(static_cast<float>((static_cast<double>(value) - static_cast<double>(a)) / (static_cast<double>(b) - static_cast<
			double>(a))));
	}
	return 0.0f;
}
